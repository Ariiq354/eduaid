<script lang="ts">
    ...
    $: selectedTP = $formData.tpId
      ? {
          label: data.tujuanPembelajaran.find((tujuanPembelajaran) => tujuanPembelajaran.id == $formData.tpId)?.tujuanPembelajaran,
          value: $formData.tpId
        }
      : undefined;
  </script>
        ...
        <ul class="flex flex-col py-8 px-4">
                    <li class="self-start text-left">
                        ...
                        <div class="px-4 py-2 rounded-r-md rounded-t-md bg-slate-600 text-white max-w-3xl w-full">
                            Halo, saya Edu Ai. Saya akan membantu Anda membuat Modul Pembelajaran. Berikan saja Tujuan Pembelajaran yang ingin Anda pilih, dan saya akan menghasilkan daftar saran untuk Anda.
                        </div>
                    </li>
            ...
            <Form.Field {form} name="tpId">
              <Form.Control let:attrs>
                ...
                >
                  ...
                  <Select.Content>
                    {#each data.tujuanPembelajaran as tujuanPembelajaran}
                      <Select.Item value={tujuanPembelajaran.id} label={tujuanPembelajaran.tujuanPembelajaran} />
                    {/each}
                  </Select.Content>
                </Select.Root>
