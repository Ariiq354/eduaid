  ...
  const form = superForm(data.form, {
    ...
    async onUpdate({ form }) {
      if (form.valid) {
        ...
        await goto(`/dashboard/modul/${data.tpId}`);
      }
    },
...
{#if data.tp}
  <div class="flex flex-col gap-4">
    ...
    <div class="flex items-center justify-between">
      ...
      <Button variant="outline" href={`/dashboard/modul/${data.tpId}`} class="p-2 shadow-lg">
        <ArrowLeft />
      </Button>
    </div>
        ...
        <ScrollArea class="h-96">
          <ul class="flex flex-col gap-2 overflow-y-auto p-4">
            <li class="self-start text-left">
              ...
              >
                Halo, saya Edu Ai. Saya akan membantu Anda membuat Modul Pembelajaran. Berikan saja
                Tujuan Pembelajaran yang ingin Anda pilih, dan saya akan menghasilkan daftar saran
                untuk Anda.
              </div>
            </li>
